<template>
  ...
  <div class="bread">
    <el-breadcrumb separator-class="el-icon-arrow-right">
      ...
      <el-breadcrumb-item>答疑回复</el-breadcrumb-item>
    </el-breadcrumb>
  </div>
...
<script>
  ...
  import { findRequestionComment } from '../../api/questions'
    export default {
        name: "RequestComment",
      ...
      created() {
        ...
        this.uid=userId.id
        this.formInline.key.uid=userId.id
        this.getQuestionsData(this.formInline)
      },
      methods:{
        async getQuestionsData(param){
          this.loading = true
          findRequestionComment(param).then(res=>{
            this.loading = false
            if(res.status!=200){
              this.$message({
                type: 'error',
              ...
              console.log("ta"+JSON.stringify(res.data.list))
        ...
        handEdit(index,row){
          this.$router.push({ name: 'questionItem',query:{qid:row.question.id} });
        },
        handleEdit2() {
