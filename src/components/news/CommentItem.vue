    ...
    <ul>
      <li v-for="i in tableData">
        <div style="padding-top: 20px;margin-left: 10px">
          ...
          <span style="float:left;display:inline-block;line-height:70px;padding-left: 5px">{{i.name}}评论了文章<<{{i.title}}>></span>
...
<script>
  ...
  import {getArticleComment,addReply } from '../../api/Article'
  ...
  export default {
    ...
    methods:{
      async getData(param){
        getArticleComment(param).then(res=>{
          if(res.status!=200){
            this.$message({
              type: 'error',
              message: "查询失败"
